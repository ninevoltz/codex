...
async fn turn_diff_display_roots(turn_context: &TurnContext) -> Vec<(String, PathBuf)> {
    if turn_context.environments.turn_environments.is_empty() {
        #[allow(deprecated)]
        let display_root = get_git_repo_root(turn_context.cwd.as_path())
            .unwrap_or_else(|| turn_context.cwd.clone().into_path_buf());
        return vec![(String::new(), display_root)];
    }

    let mut display_roots = Vec::with_capacity(turn_context.environments.turn_environments.len());
    for turn_environment in &turn_context.environments.turn_environments {
        let display_root = get_git_repo_root_with_fs(
        ...
        .into_path_buf();
        display_roots.push((turn_environment.environment_id.clone(), display_root));
    }
    display_roots
}
